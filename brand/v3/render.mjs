// Renders v3 PNGs. usage: python brand/v3/make.py && node brand/v3/render.mjs
...
for (const [src, out, w] of [['logo-paper.svg', 'logo.png', 2000], ['icon.svg', 'icon-1080.png', 1080], ['logo-reversed.svg', 'logo-reversed.png', 2000]]) {
  await page.setViewport({ width: w, height: w });
  await page.setContent(`<style>*{margin:0}img{display:block;width:${w}px;height:${w}px}</style><img src="${url(src)}">`, { waitUntil: 'load' });
  await page.screenshot({ path: path.join(dir, out), omitBackground: true, clip: { x: 0, y: 0, width: w, height: w } });
}
await browser.close();
